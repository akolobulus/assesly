...
>(
  (
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, refetchFolders]
    )
  ...
  (
    ...
    return (
      <div
        ...
        variant === "sidebar" && state === "collapsed" && "data-[collapsible=icon]"
        )}
        data-state={state}
      ...
      >
        <div
          className={cn(
            ...
            variant === "floating" || variant === "inset"
              ? state === "collapsed" && collapsible === "icon" && "w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4))]"
        ...
        <div
          className={cn(
            ...
            variant === "floating" || variant === "inset"
              ? state === "collapsed" && collapsible === "icon" ? "p-2 w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]" : "p-2"
              : state === "collapsed" && collapsible === "icon" ? "w-[--sidebar-width-icon]" : "",
...
>(
  (
    {
      asChild: ownAsChild = false, // Use ownAsChild to distinguish from props.asChild
      isActive = false,
      variant = "default",
      size = "default",
      tooltip,
      ...
      ...props // Remaining props, which might include asChild from a parent like Link
    },
    ref
  ) => {
    ...
    // Props to be passed to the Comp (Slot or button)
    // These include props from the consumer of SidebarMenuButton (like href from Link)
    // AND internal data attributes of SidebarMenuButton.
    const compProps: React.ComponentProps<typeof Comp> & { [key: string]: any } = {
      ...props, // Spread consumer props first
      ref: ref,
    ...
    // If Comp is a "button" (i.e., ownAsChild is false),
    // we must ensure that an 'asChild' prop from consumer props is not passed to the <button> element.
    if (!ownAsChild && typeof compProps.asChild !== 'undefined') {
